...
public class Context {
    ...
    public void removeTemp(UUID context, String key){
        storage.computeIfAbsent(context, (ignored) -> new ExpiringMap<>()).remove(key);
    }
    ...
    public class ExpiringMap<K, V> implements Map<K, V> {
        ...
        public Pair<K, V> cleanup(Object findKey, Object findValue, Long removing) {
            long currentMillis = getCurrentMillis();
            Iterator<Entry<K, Long>> expireIter = this.birth.entrySet().iterator();

            while (expireIter.hasNext()) {
                Entry<K, Long> entry = expireIter.next();
                K key = entry.getKey();
